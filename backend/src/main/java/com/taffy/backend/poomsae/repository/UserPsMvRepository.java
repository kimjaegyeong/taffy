...
//    List<UserPsMv> findByUserIdAndPsMvId(@Param("userId") Integer userId, @Param("psMvId") Integer psMvId);
    ...
    @Query("SELECT new com.taffy.backend.poomsae.dto.MvDetailDto(pm.psMvId, m.mvId, pm.psMvSeq, " +
            ...
            "LEFT JOIN UserPsMv upm ON upm.psMv.psMvId = pm.psMvId AND upm.member.id = :userId " +
            "WHERE pm.ps.psId" +
            " = :psId AND pm.psMvSeq = :mvSeq")
    MvDetailDto findMvDetail(@Param("userId") Long userId, @Param("psId") Integer psId, Integer mvSeq);

//    @Query("SELECT upm FROM UserPsMv upm WHERE upm.member.id = :userId AND upm.psMv.psId = :psId AND")
//    Optional<UserPsMv> findByUserIdAndPsMvId(@Param("userId") Long userId, @Param("psId") Integer psId, @Param("mvSeq") Integer mvSeq);

    @Query("SELECT upm FROM UserPsMv upm JOIN PsMv pm ON upm.psMv.psMvId = pm.psMvId WHERE upm.member.id = :userId AND pm.ps.psId = :psId AND pm.psMvSeq = :mvSeq")
    Optional<UserPsMv> findByUserIdAndPsIdAndMvSeq(@Param("userId") Long userId, @Param("psId") Integer psId, @Param("mvSeq") Integer mvSeq);
